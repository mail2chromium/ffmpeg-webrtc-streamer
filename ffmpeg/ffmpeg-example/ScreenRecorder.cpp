		...
		if(pAVPacket->stream_index == VideoStreamIndx)
		{
			...
			if(frameFinished)// Frame successfully decoded :)
			{
				...
				if(got_picture)
				{
					...
					printf("Write frame %3d (size= %2d), pts=%lld, dts=%lld\n", j++,
							outPacket.size/1000, outPacket.pts, outPacket.dts);
...
int ScreenRecorder::openCamera()
{
	...
	value = av_dict_set( &options,"framerate","5",0 );
	if(value < 0)
	{
...
*/
	 ...
	 * AVFoundation video devices:
		[AVFoundation input device @ 0x7f99e9502a40] [0] FaceTime HD Camera
		[AVFoundation input device @ 0x7f99e9502a40] [1] Capture screen 0
	 */
  	value = avformat_open_input(&pAVFormatContext, "Capture screen 0", pAVInputFormat, &options);
	if(value != 0)
	{
...
/* Returns the output format in the list of registered output formats which best matches the provided parameters, or returns NULL if there is no match. */
	...
	video_st->codec = outAVCodecContext;
//	outAVCodecContext = video_st->codec;
	...
	outAVCodecContext->width = 1280;
	outAVCodecContext->height = 720;
...
//	 av_opt_set(outAVCodecContext->priv_data, "preset", "slow", 0);
		...
		// 实时编码关键看这句，上面那条无所谓
		av_opt_set(outAVCodecContext->priv_data, "tune", "zerolatency", 0);
		av_opt_set(outAVCodecContext->priv_data, "profile", "main", 0);
	}
