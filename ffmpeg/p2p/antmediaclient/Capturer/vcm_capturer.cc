...
bool VideoCodecIsRequested = false;
bool RequestedVideoTrack = false;

std::vector<uint8_t> import_buff_4_video;
int WebSocketHelper::raw_data = 1;
...
void SendVideoPackets(int pkt_size) {
...
//    RTC_LOG(LS_NONE) << pkt_size << ": Inside Callback Function";
    ...
    if(!peer_connection)
        ...
        if (peer_connection->peer_connection_state() != webrtc::PeerConnectionInterface::PeerConnectionState::kConnected)
            return;
    }
    ...
    if (WebSocketHelper::raw_data == 1) {
        ...
        const int conversionResult = libyuv::ConvertToI420(
                import_buff_4_video.data(), pkt_size, buffer.get()->MutableDataY(),
        ...
        buffer->GetBuffer().assign(import_buff_4_video.data(),
                                   import_buff_4_video.data() + pkt_size);
...
void FillVideoPackets(int pos, uint8_t data) {
    import_buff_4_video.push_back(data);
}

void DeleteVideoPackets() {
    import_buff_4_video.clear();
}
