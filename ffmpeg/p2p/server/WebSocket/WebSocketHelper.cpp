...
//        wav_out += std::to_string(import_buff.at(j));
//        if (j != pkt_size - 1)
//            wav_out += " ";
//    }
//    RTC_LOG(LS_ERROR) << pkt_size << " bytes: " << wav_out.c_str();
//
//    if (AudioCapturer) {
//        RTC_LOG(LS_ERROR) << "Audio capturer initiated";
//        AudioCapturer->ProcessPacket(import_buff.data(), pkt_size);
//    } else {
...
void display_pkt_type(int type) {
    RTC_LOG(LS_ERROR) << " packet type : " << type;
}
